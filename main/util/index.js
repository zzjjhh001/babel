    ...
    if (pathList.includes('pages')) {
      ...
      for(item of _path) {
        ...
        if(isPage(currentPath)) {
          ...
          paths.push(getName(currentPath))
        } else {
...
function getName(path) {
  let t;
  const file = fs.readdirSync(path, (err, file) => {
    if(err) {
      return [];
    }
    return file;
  ...
  for( item of file ) {
    if( item.slice(-5) === '.wxml' ) {
      t = item.slice(0, -5);
    }
  }
  return path + '/' + t; // todo 删除前边多余的字符 
}
// 获取page下的文件
function page(path) {
  let pathList = [];
...
function getAllpages(path) {
  return pages(path + '/pages').concat(packages(path + '/package'), page(path + '/page'));
}
module.exports = { isPage, getFiles, pages, packages, getAllpages, getName };